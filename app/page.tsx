...
export default function Page() {
  ...
  const { submitUserMessage, get_events, list_stocks } = useActions<typeof AI>();
  ...
  const handleShowStockPurchaseUI = async () => {
    // For now, use the same function as submitUserMessage
    // You can replace this with the actual function for showing stock purchase UI
    await submitUserMessage('Show stock purchase UI');
  };

  const handleListStocks = async () => {
    // Call the list_stocks action
    try {
      const responseMessage = await list_stocks([]);
      setMessages((currentMessages: any) => [...currentMessages, responseMessage]);
    } catch (error) {
      console.error(error);
    }
